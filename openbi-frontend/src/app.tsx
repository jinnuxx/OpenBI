...
export async function getInitialState(): Promise<{
  ...
  if (location.pathname !== loginPath) {
    ...
    return {
      currentUser,
      settings: settings, // 将 settings 添加到返回值中
    };
  }
...
export const layout: RunTimeLayoutConfig = ({ initialState, setInitialState }) => {
  ...
  return {
    ...
    // waterMarkProps: {
    //   content: initialState?.currentUser?.userName || '', // 显式指定类型
    // },
    footerRender: () => <Footer />,
    onPageChange: () => {
      const { location } = history;
...
export const request = {
  // baseURL: 'http://localhost:8101',//122.51.230.61
  baseURL: 'http://122.51.230.61:8101',
