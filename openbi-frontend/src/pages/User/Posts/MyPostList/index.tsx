...
const Resources = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await listMyPostVOByPageUsingPOST({ current: currentPage, pageSize: pageSize });
    ...
    const handleSearchMy = async () => {
        try {
            const response = await listMyPostVOByPageUsingPOST({
                current: currentPage,
                pageSize: pageSize,
                content: searchQuery,
    ...
    return (
        ...
        >
            ...
            <Input.Search
                placeholder="Search my posts by content..."
                value={searchQuery}
            ...
            <Row gutter={16}>
                {posts.map((post) => (
                    <Col span={12} key={post.id}>
                        <Card
                            title={
                                <Link to={`/post/myPostContent/${post.id}`}>
                                    {post.title}
                                </Link>
                            }
                            bordered={false}
